...
def main():
  ...
  print('model_type: {}\tscale:{}\tBeta distribution: Beta({})\tlr: {}\t \
         pert_coeff: {}'.format(model_type, scale, beta_param, lr, pert_coeff))
  ...
  model = Net(input_dim, output_dim, rule_encoder, data_encoder,
              hidden_dim=hidden_dim_db, n_layers=n_layers,
              merge=merge).to(device)    # Not residual connection
  ...
  saved_filename = 'cardio_{}_rule-{}_src{}-\
                    target{}_seed{}.demo.pt'
                    .format(model_type, rule_feature,
                            src_usual_ratio, src_usual_ratio, seed)
  ...
  for epoch in range(1, epochs+1):
    ...
    for batch_train_x, batch_train_y in train_loader:
      ...
      pert_batch_train_x[:, rule_ind] = \
          get_perturbed_input(pert_batch_train_x[:, rule_ind], pert_coeff)
      pert_output = model(pert_batch_train_x, alpha=alpha)

      # output should be less than pert_output
      loss_rule = loss_rule_func(output, pert_output)
    ...
    if epoch % valid_freq == 0:
      ...
      with torch.no_grad():
        for val_x, val_y in valid_loader:
          ...
          pert_val_x[:, rule_ind] = get_perturbed_input(pert_val_x[:, rule_ind],
                                                        pert_coeff)
          ...
          val_ratio = verification(pert_output, output,
                                   threshold=0.0).item()
        ...
        if val_loss < best_val_loss:
          ...
          print('[Valid] Epoch: {} Loss: {:.6f} (alpha: {:.2f})\t \
                Loss(Task): {:.6f} Acc: {:.2f}\t Loss(Rule): {:.6f}\t \
                Ratio: {:.4f} best model is updated %%%%'
                .format(epoch, best_val_loss, alpha,
                        val_loss_task, val_acc, val_loss_rule, val_ratio))
          torch.save({
              'epoch': epoch,
          ...
          print('[Valid] Epoch: {} Loss: {:.6f} (alpha: {:.2f})\t \
                Loss(Task): {:.6f} Acc: {:.2f}\t Loss(Rule): {:.6f}\t \
                Ratio: {:.4f}({}/{})'
                .format(epoch, val_loss, alpha,
                        val_loss_task, val_acc, val_loss_rule, val_ratio,
                        counter_early_stopping, early_stopping_thld))
  ...
  rule_encoder = RuleEncoder(input_dim, output_dim_encoder, hidden_dim_encoder)
  data_encoder = DataEncoder(input_dim, output_dim_encoder, hidden_dim_encoder)
  model_eval = Net(input_dim, output_dim, rule_encoder, data_encoder,
                   hidden_dim=hidden_dim_db, n_layers=n_layers,
                   merge=merge).to(device)

  checkpoint = torch.load(saved_filename)
  model_eval.load_state_dict(checkpoint['model_state_dict'])
  print("best model loss: {:.6f}\t at epoch: {}"
  ...
  pert_test_x[:, rule_ind] = get_perturbed_input(pert_test_x[:, rule_ind],
                                                 pert_coeff)
  for alpha in alphas:
    model_eval.eval()
    with torch.no_grad():
      ...
      test_ratio = verification(pert_output, output,
                                threshold=0.0).item()
    ...
    print('[Test] Average loss: {:.8f} (alpha:{})'
          .format(test_loss_task, alpha))
    print('[Test] Accuracy: {:.4f} (alpha:{})'
          .format(test_acc, alpha))
    print("[Test] Ratio of verified predictions: {:.6f} (alpha:{})"
          .format(test_ratio, alpha))
    print()
